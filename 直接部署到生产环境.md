# 🚀 直接部署到生产环境（跳过本地测试）

## ✅ 为什么直接部署更好？

1. **避免本地环境问题**
   - 不需要处理 HTTPS/HTTP 混用
   - 不需要处理 Shopify CLI 代理
   - 不需要处理 localhost 证书问题

2. **更接近生产环境**
   - 使用真实的 HTTPS URL
   - OAuth 流程更稳定
   - 更容易调试

3. **节省时间**
   - 不需要反复调试本地环境
   - 部署后直接测试

## 📋 部署步骤

### 步骤1：准备部署环境

#### 1.1 创建 Vercel 账户（如果还没有）

访问：https://vercel.com/signup

#### 1.2 创建 Supabase 账户（用于 PostgreSQL 数据库）

访问：https://supabase.com/dashboard

### 步骤2：部署数据库到 Supabase

1. **创建新项目**
   - 登录 Supabase Dashboard
   - 点击 "New Project"
   - 填写项目信息
   - 记录数据库连接字符串（格式：`postgresql://...`）

2. **更新 Prisma Schema**

   将 `prisma/schema.prisma` 中的 `datasource` 改为 PostgreSQL：

   ```prisma
   datasource db {
     provider = "postgresql"
     url      = env("DATABASE_URL")
   }
   ```

3. **运行数据库迁移**

   ```bash
   cd /Users/Zhuanz/cursor/resinmemory_product/ezproduct
   # 设置 Supabase 数据库 URL
   export DATABASE_URL="你的Supabase数据库连接字符串"
   npm run db:migrate
   ```

### 步骤3：部署应用到 Vercel

#### 3.1 安装 Vercel CLI（如果还没有）

```bash
npm install -g vercel
```

#### 3.2 登录 Vercel

```bash
vercel login
```

#### 3.3 部署应用

```bash
cd /Users/Zhuanz/cursor/resinmemory_product/ezproduct
vercel
```

按照提示：
- 选择项目名称（或使用默认）
- 选择框架：Remix
- 确认设置

#### 3.4 配置环境变量

在 Vercel Dashboard 中，进入项目设置 → Environment Variables，添加：

```
SHOPIFY_API_KEY=你的Shopify_API_Key
SHOPIFY_API_SECRET=你的Shopify_API_Secret
SCOPES=write_products,read_products,write_product_listings,read_product_listings
SHOPIFY_APP_URL=https://你的应用.vercel.app
DATABASE_URL=你的Supabase数据库连接字符串
GEMINI_API_KEY=你的Gemini_API_Key
NODE_ENV=production
```

### 步骤4：更新 Shopify Partners Dashboard 配置

1. **进入应用设置**
   - 登录 https://partners.shopify.com/
   - 进入你的应用
   - 进入 "App setup" 或 "应用设置"

2. **更新 App URL**
   - **App URL**: `https://你的应用.vercel.app`
   - 保存

3. **更新 Redirect URLs**
   - **Allowed redirection URL(s)**:
     ```
     https://你的应用.vercel.app/auth/callback
     https://你的应用.vercel.app/auth/shopify/callback
     https://你的应用.vercel.app/api/auth/callback
     ```
   - 保存

4. **更新应用版本配置**
   - 进入 "Versions" → 你的版本
   - 更新 **App URL** 为 `https://你的应用.vercel.app`
   - 更新 **Redirect URLs** 为上面的三个 URL
   - 保存并发布

### 步骤5：更新 shopify.app.toml

```toml
name = "EZProduct"
client_id = "46a6a6c60a57cd723019c930a072aa10"
application_url = "https://你的应用.vercel.app"
embedded = true

[access_scopes]
scopes = "write_products,read_products,write_product_listings,read_product_listings"

[auth]
redirect_urls = [
  "https://你的应用.vercel.app/auth/callback",
  "https://你的应用.vercel.app/auth/shopify/callback",
  "https://你的应用.vercel.app/api/auth/callback"
]
```

### 步骤6：完成 OAuth 安装

1. **在 Shopify 后台安装应用**
   - 访问：`https://你的商店.myshopify.com/admin/apps`
   - 找到 "EZProduct" 应用
   - 点击 "安装" 或 "Install"

2. **完成授权**
   - 点击 "Install" 按钮
   - 完成 OAuth 流程

3. **测试应用**
   - 打开应用界面
   - 输入产品关键词
   - 测试产品生成功能

## 🎯 快速部署命令总结

```bash
# 1. 安装 Vercel CLI
npm install -g vercel

# 2. 登录 Vercel
vercel login

# 3. 部署应用
cd /Users/Zhuanz/cursor/resinmemory_product/ezproduct
vercel

# 4. 在 Vercel Dashboard 中配置环境变量
# 5. 在 Shopify Partners Dashboard 中更新 URL
# 6. 在 Shopify 后台安装应用
```

## 📝 注意事项

1. **数据库迁移**
   - 部署前确保数据库迁移已运行
   - 或者使用 Vercel 的 Build Command 自动运行迁移

2. **环境变量**
   - 确保所有环境变量都已配置
   - 特别是 `DATABASE_URL` 和 `SHOPIFY_API_SECRET`

3. **URL 更新**
   - 确保 Shopify Partners Dashboard 中的 URL 与 Vercel 部署的 URL 一致
   - 更新后需要等待几分钟生效

## ✅ 部署后的优势

- ✅ 使用真实的 HTTPS URL
- ✅ OAuth 流程更稳定
- ✅ 不需要处理本地环境问题
- ✅ 更容易调试和测试
- ✅ 可以随时访问应用




