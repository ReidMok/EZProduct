# 🔧 解决数据库连接错误

## ⚠️ 错误

```
Error: P1001: Can't reach database server at `db.cugxiuizyhalmdxekged.supabase.co:5432`
```

## 🔍 可能的原因

1. **密码 URL 编码问题**
   - 特殊字符可能没有正确编码
   - 或者编码方式不对

2. **Supabase 数据库未启用**
   - 项目可能还在创建中
   - 或者数据库被暂停

3. **网络连接问题**
   - 防火墙阻止连接
   - VPN 干扰

## ✅ 解决方案

### 方案 1：检查 Supabase 项目状态

1. **访问 Supabase Dashboard**
   - 打开：https://supabase.com/dashboard
   - 进入你的项目 `EZProduct`

2. **检查项目状态**
   - 确认项目状态是 "Active"（不是 "Paused"）
   - 如果被暂停，点击 "Resume" 恢复

3. **检查数据库设置**
   - 点击左侧 "Settings" → "Database"
   - 确认数据库已创建完成

### 方案 2：使用 Supabase 提供的连接字符串

1. **在 Supabase Dashboard**
   - Settings → Database
   - 找到 "Connection string"
   - 选择 "URI" 标签
   - **直接复制完整的连接字符串**（不要手动替换密码）

2. **使用复制的连接字符串**
   ```bash
   DATABASE_URL="粘贴完整的连接字符串" npm run db:migrate
   ```

### 方案 3：检查密码编码

如果 Supabase 提供的连接字符串格式是：
```
postgresql://postgres:[YOUR-PASSWORD]@db.xxx.supabase.co:5432/postgres
```

你需要：
1. 将 `[YOUR-PASSWORD]` 替换为实际密码：`&hmB9!2_UK#7%?8`
2. 然后对整个密码进行 URL 编码

**或者更简单的方法：**
- 直接使用 Supabase Dashboard 提供的连接字符串
- 它应该已经正确编码了

### 方案 4：使用 Supabase SQL Editor 创建表

如果连接仍然失败，可以在 Supabase Dashboard 中手动创建表：

1. **打开 SQL Editor**
   - 在 Supabase Dashboard 中点击左侧 "SQL Editor"

2. **运行以下 SQL**：
   ```sql
   -- 创建 Shop 表
   CREATE TABLE "Shop" (
     "id" TEXT NOT NULL,
     "shop" TEXT NOT NULL,
     "accessToken" TEXT NOT NULL,
     "scope" TEXT NOT NULL,
     "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
     "updatedAt" TIMESTAMP(3) NOT NULL,
     CONSTRAINT "Shop_pkey" PRIMARY KEY ("id")
   );

   CREATE UNIQUE INDEX "Shop_shop_key" ON "Shop"("shop");
   CREATE INDEX "Shop_shop_idx" ON "Shop"("shop");

   -- 创建 ProductGeneration 表
   CREATE TABLE "ProductGeneration" (
     "id" TEXT NOT NULL,
     "shopId" TEXT NOT NULL,
     "keywords" TEXT NOT NULL,
     "imageUrl" TEXT,
     "title" TEXT NOT NULL,
     "descriptionHtml" TEXT NOT NULL,
     "tags" TEXT NOT NULL,
     "seoTitle" TEXT,
     "seoDescription" TEXT,
     "variantsJson" TEXT NOT NULL,
     "shopifyProductId" TEXT,
     "shopifyProductHandle" TEXT,
     "status" TEXT NOT NULL DEFAULT 'pending',
     "errorMessage" TEXT,
     "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
     "updatedAt" TIMESTAMP(3) NOT NULL,
     CONSTRAINT "ProductGeneration_pkey" PRIMARY KEY ("id")
   );

   CREATE INDEX "ProductGeneration_shopId_idx" ON "ProductGeneration"("shopId");
   CREATE INDEX "ProductGeneration_status_idx" ON "ProductGeneration"("status");
   CREATE INDEX "ProductGeneration_createdAt_idx" ON "ProductGeneration"("createdAt");

   ALTER TABLE "ProductGeneration" ADD CONSTRAINT "ProductGeneration_shopId_fkey" FOREIGN KEY ("shopId") REFERENCES "Shop"("id") ON DELETE CASCADE ON UPDATE CASCADE;
   ```

3. **点击 "Run" 执行 SQL**

## 🔍 诊断步骤

### 1. 测试数据库连接

在 Supabase Dashboard 中：
- Settings → Database
- 点击 "Test connection"
- 看看是否能成功连接

### 2. 检查项目状态

确认项目：
- 状态是 "Active"
- 数据库已创建完成
- 没有暂停或限制

### 3. 使用 Supabase 提供的连接字符串

**重要**：不要手动编码密码，直接使用 Supabase Dashboard 提供的连接字符串，它应该已经正确格式化了。

---

先检查 Supabase 项目状态，然后使用 Supabase Dashboard 提供的连接字符串重新尝试！




